...
def read_graph(args):
    """
    Method to read graph and create positive and negative adjacency matrices (AP and AN).
    :param args: Arguments object.
    :return edges: Dictionary containing positive and negative adjacency matrices and other info.
    """
    dataset = pd.read_csv(args.edge_path).values.tolist()

    # Count the number of nodes
    nodes = set([edge[0] for edge in dataset] + [edge[1] for edge in dataset])
    ncount = len(nodes)
    
    # Create lists to store positive and negative edges
    positive_edges = [edge[0:2] for edge in dataset if edge[2] == 1]
    negative_edges = [edge[0:2] for edge in dataset if edge[2] == -1]

    # Create positive and negative adjacency matrices
    row_pos = [edge[0] for edge in positive_edges]
    col_pos = [edge[1] for edge in positive_edges]
    data_pos = [1] * len(positive_edges)
    
    row_neg = [edge[0] for edge in negative_edges]
    col_neg = [edge[1] for edge in negative_edges]
    data_neg = [1] * len(negative_edges)

    # Create sparse adjacency matrices A_P and A_N
    A_P = csr_matrix((data_pos, (row_pos, col_pos)), shape=(ncount, ncount))
    A_N = csr_matrix((data_neg, (row_neg, col_neg)), shape=(ncount, ncount))

    edges = {
        "positive_edges": positive_edges,
        "negative_edges": negative_edges,
        "positive_adjacency_matrix": A_P,
        "negative_adjacency_matrix": A_N,
        "ecount": len(dataset),
        "ncount": ncount
    }
